...
$(function () {
    ...
    $.getJSON("data/message.json" , function (results) {
        ...
        let arr = messages.slice((step-1) , step+4-1);
...
//监听滚动
    $("#container").scroll(function () {   //监听滚动事件..
        ...
        if (container_totalHeight == Math.ceil(contianer_scroll) + Math.ceil(container_height)) {

            setTimeout(function () {
                let arr = messages.slice((step-1) , step+4-1);   //splice 在原数组基础上删除某些元素 , slice 修改数组并返回副本 , 原本不动
                createItem(arr);
                step += 4;
            }, 200)
